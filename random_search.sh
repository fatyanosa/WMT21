...
do
    ...
    fairseq-train 'dataset/data-bin/flores101.jv_id_ms_tl_ta_en.bpe' --finetune-from-model flores101_mm100_175M/model.pt --save-dir checkpoints/flores101_mm100_175M/"$i" --task translation_multi_simple_epoch --encoder-normalize-before --langs 'en,id,jv,ms,ta,tl' --lang-pairs 'en-id,id-en,en-jv,jv-en,en-ms,ms-en,en-ta,ta-en,en-tl,tl-en,id-jv,jv-id,id-ms,ms-id,id-ta,ta-id,id-tl,tl-id,jv-ms,ms-jv,jv-ta,ta-jv,jv-tl,tl-jv,ms-ta,ta-ms,ms-tl,tl-ms,ta-tl,tl-ta' --max-tokens 512 --decoder-normalize-before --sampling-method temperature --sampling-temperature 1.5 --encoder-langtok src --decoder-langtok --criterion label_smoothed_cross_entropy --label-smoothing 0.2 --optimizer adam --adam-eps ${epsilon} --adam-betas "(${beta1}, ${beta2})" --lr-scheduler inverse_sqrt --lr ${learning_rate} --warmup-updates 2500 --max-epoch 3 --dropout ${dropout} --attention-dropout ${attention_dropout} --weight-decay ${weight_decay} --update-freq 4 --seed ${seed} --log-format simple --log-interval 2 --arch transformer_wmt_en_de_big  --share-decoder-input-output-embed --share-all-embeddings --ddp-backend c10d  --no-epoch-checkpoints --num-workers 4 --batch-size ${batch_size} --empty-cache-freq 1 --wandb-project WMT21 --combine-val --pipeline-encoder-balance 6 --pipeline-encoder-balance 6 --scoring sacrebleu --save-interval 1 --save-interval-updates 1000 --keep-interval-updates 1
